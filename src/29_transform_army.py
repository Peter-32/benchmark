...
pro_df = local.read.csv('27_extract_pro_army', 'interim')
pro_df = pro_df.query("game_length_seconds_max_600 >= 420")
my_df = local.read.csv('28_extract_my_army', 'interim')
...
for game_number in pro_df.game_number.unique():
    ...
    input_data1 = list(pro_df_one_game.sort_values(by='army_time')['army_time'].values)
    input_data2 = list(pro_df_one_game.sort_values(by='army_time')['army_amount'].values)
    ...
    pro_data_cumulative_df['army_amount'] = cumulative_total
...
local.write.csv(pro_df, '29_pro_army_transform')


dfs = []
for game_number in my_df.game_number.unique():
    my_df_one_game = my_df.query(f'game_number == {game_number}')
    input_data1 = list(my_df_one_game.sort_values(by='army_time')['army_time'].values)
    input_data2 = list(my_df_one_game.sort_values(by='army_time')['army_amount'].values)
...
local.write.csv(my_df, '29_my_army_transform')
