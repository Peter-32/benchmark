...
pro_terran_aggregated_data = pro_terran_df.groupby('second')['supply_amount'].quantile(0.80)[:-50]
...
plt.legend(['my cumulative supply gap', 'pro cumulative supply gap'])
plt.title(f"supply ZvT Benchmark - Score {score:.0%}")
plt.savefig(f"png_my_supply_zvt_benchmark.png")
plt.clf()
...
pro_zerg_aggregated_data = pro_zerg_df.groupby('second')['supply_amount'].quantile(0.80)[:-50]
...
plt.savefig(f"png_my_supply_zvz_benchmark.png")
plt.clf()
...
pro_protoss_aggregated_data = pro_protoss_df.groupby('second')['supply_amount'].quantile(0.80)[:-50]
...
plt.savefig(f"png_my_supply_zvp_benchmark.png")
plt.clf()
