...
pro_df = local.read.csv('15_extract_pro_creeps', 'interim')
pro_df = pro_df.query("game_length_seconds_max_600 >= 420")
my_df = local.read.csv('16_extract_my_creeps', 'interim')
...
for game_number in pro_df.game_number.unique():
    ...
    input_data = list(pro_df_one_game.sort_values(by='creep_time')['creep_time'].values)
    ...
    pro_data_cumulative_df['creep_count'] = cumulative_count
...
local.write.csv(pro_df_output, '17_pro_creep_transform')


dfs = []
for game_number in my_df.game_number.unique():
    my_df_one_game = my_df.query(f'game_number == {game_number}')
    input_data = list(my_df_one_game.sort_values(by='creep_time')['creep_time'].values)
...
local.write.csv(my_df_output, '17_my_creep_transform')
