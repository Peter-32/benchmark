...
my_terran_aggregated_data = my_terran_df.groupby('second')['inject_count'].quantile(0.50)[:-50]
...
pro_terran_aggregated_data = pro_terran_df.groupby('second')['inject_count'].quantile(0.80)[:-50]
...
my_zerg_aggregated_data = my_zerg_df.groupby('second')['inject_count'].quantile(0.50)[:-50]
my_zerg_aggregated_data

pro_zerg_df = pro_df.query('opponent_race == "Zerg"')
pro_zerg_aggregated_data = pro_zerg_df.groupby('second')['inject_count'].quantile(0.80)[:-50]
...
my_protoss_aggregated_data = my_protoss_df.groupby('second')['inject_count'].quantile(0.50)[:-50]
my_protoss_aggregated_data

pro_protoss_df = pro_df.query('opponent_race == "Protoss"')
pro_protoss_aggregated_data = pro_protoss_df.groupby('second')['inject_count'].quantile(0.80)[:-50]
