...
my_terran_aggregated_data = my_terran_df.groupby('second')['larva_amount'].quantile(0.50)[:-50]
...
pro_terran_aggregated_data = pro_terran_df.groupby('second')['larva_amount'].quantile(0.20)[:-50]
...
plt.savefig(f"png_my_larva_zvt_benchmark.png")
plt.clf()

my_zerg_df = my_df.query('opponent_race == "Zerg"')
my_zerg_aggregated_data = my_zerg_df.groupby('second')['larva_amount'].quantile(0.50)[:-50]
my_zerg_aggregated_data

pro_zerg_df = pro_df.query('opponent_race == "Zerg"')
pro_zerg_aggregated_data = pro_zerg_df.groupby('second')['larva_amount'].quantile(0.20)[:-50]
...
plt.savefig(f"png_my_larva_zvz_benchmark.png")
plt.clf()

my_protoss_df = my_df.query('opponent_race == "Protoss"')
my_protoss_aggregated_data = my_protoss_df.groupby('second')['larva_amount'].quantile(0.50)[:-50]
my_protoss_aggregated_data

pro_protoss_df = pro_df.query('opponent_race == "Protoss"')
pro_protoss_aggregated_data = pro_protoss_df.groupby('second')['larva_amount'].quantile(0.20)[:-50]
...
plt.savefig(f"png_my_larva_zvp_benchmark.png")
plt.clf()
